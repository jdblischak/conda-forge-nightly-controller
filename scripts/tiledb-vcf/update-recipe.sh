...
# Update version by appending the date
the_date="$(cat date.txt)"
sed -i s/"{% set version = \"\(.*\)\" %}"/"{% set version = \"\\1.${the_date}\" %}"/ \
  tiledb-vcf-feedstock/recipe/meta.yaml
...
# Print differences
git -C tiledb-vcf-feedstock/ --no-pager diff recipe/meta.yaml
